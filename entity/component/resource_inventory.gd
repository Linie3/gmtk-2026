...
func set_resource(resource_type: GameResource.ResourceType, amount: int) -> void:
	resources[resource_type] = amount
	resource_changed.emit(resource_type, resources[resource_type])

func clear_resources() -> void:
	remove_resources(get_resources())
...
func get_resources() -> Dictionary[GameResource.ResourceType, int]:
	return resources