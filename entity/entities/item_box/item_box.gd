...
var resource_container_scene: PackedScene = preload("uid://bivbb5iw5v2te")
...
func set_item(new_item: Item) -> void:
	...
	sprite.texture = GameItem.get_item_icon(item.item_name)
...
func on_resource_changed(resource_type: GameResource.ResourceType, amount: int) -> void:
	...
	if player_resource_inventory.has_resources(item.repair_cost) and interactable.player_in_range:
